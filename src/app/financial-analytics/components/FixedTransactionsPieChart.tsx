...
const COLORS = [
    '#06b6d4', '#f97316', '#14b8a6', '#8b5cf6', '#eab308',
    '#ec4899', '#3b82f6', '#22c55e', '#ef4444', '#a855f7',
    '#64748b', '#78716c', '#6366f1', '#f59e0b', '#10b981',
];
...
const FixedTransactionsPieChart = ({ month, year, category }: { month: number; year: number; category?: string }) => {
    ...
    if (filtered.length === 0) {
        return <div className="text-center py-10 px-4 text-text-secondary text-sm">{t.analytics.noFixedTransactions} {monthLabel}</div>;
    }
    ...
    return (
        <>
            ...
            {allExpenseCategories.length > 0 && (
                <div className="bg-surface border border-gray-300 rounded-xl p-5">
                    <h3 className="text-[0.95rem] font-semibold text-text-primary mt-0 mb-4">{t.analytics.fixedExpensesByCategory}</h3>
