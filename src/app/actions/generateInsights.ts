...
export async function generateFinancialInsights(locale: string = 'pt'): Promise<InsightsResponse> {
    try {
        ...
        const currentFormatted = formatTransactionsForPrompt(currentMonthTransactions as unknown as PopulatedTransaction[]);
        const previousFormatted = formatTransactionsForPrompt(previousTransactions as unknown as PopulatedTransaction[]);
